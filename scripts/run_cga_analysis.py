"""
CGA analysis script for Experiments 4 and 5.
Reads pre-extracted features from notebook 01 and computes critical slowing
down indicators (Exp 4) and incremental logistic regression (Exp 5).
"""
...
def load_cga_features():
    """Load pre-extracted CGA features from parquet files."""
    utt_df = pd.read_parquet(CGA_FEATURES_PATH)
    summary_df = pd.read_parquet(CGA_CONVERSATION_SUMMARY_PATH)

    # Recompute q_da from da_prob_* columns.
    # After notebook 01 fix, columns use tag names (da_prob_aa, da_prob_bk, etc.)
    # But also handle old format (da_prob_LABEL_2, da_prob_LABEL_10, etc.)
    conciliatory_cols = []

    # Prefer tag-name columns (from fixed notebook 01)
    for tag in ["aa", "bk", "br", "ba"]:
        col = f"da_prob_{tag}"
        if col in utt_df.columns:
            conciliatory_cols.append(col)

    # Fallback: LABEL_i columns (from old notebook 01 output)
...
def compute_lexical_divergence(texts_speaker1, texts_speaker2):
    """Compute mean 1-Jaccard between consecutive speaker pair utterances."""
    divergences = []
...
def experiment_4_csd_indicators(utt_df):
    """
    Experiment 4: Critical slowing down analysis.
    Computes rolling AC1 and variance for repair proxies and AMD,
    then Kendall tau trends in pre-breakdown windows.
    """
    print("=" * 72)
    print("EXPERIMENT 4: Critical Slowing Down Indicators")
    print("=" * 72)
    ...
    for convo_id, group in utt_df.groupby("convo_id"):
        ...
        if is_derailing and pd.notna(attack_turn):
            ...
            pre_breakdown_start = max(0, pre_breakdown_end - 5)
        else:
...
def experiment_5_incremental_regression(utt_df):
    """
    Experiment 5: Incremental logistic regression for derailment prediction.
    Uses features from the first 60% of turns in each conversation.

    Feature engineering informed by Exp 4 findings:
    - Baseline: toxicity + sentiment trends + means
    - CSD indicators: rolling variance and autocorrelation trends (Kendall tau)
      for q_DA and AMD variance, which were significant in Exp 4
    - Lexical features: divergence between speakers
    """
    print("\n" + "=" * 72)
    print("EXPERIMENT 5: Incremental Logistic Regression")
    print("=" * 72)
    ...
    for convo_id, group in utt_df.groupby("convo_id"):
        ...
        # --- Baseline features: trends AND means ---
        ...
        # --- CSD: q_DA autocorrelation and variance trends (Kendall tau) ---
        ...
        # --- CSD: AMD variance trend (Kendall tau) ---
        # This was the strongest indicator in Exp 4 (p=0.001)
        if len(ge_columns) > 0:
        ...
        # --- Sentiment autocorrelation trend ---
    ...
    # Incremental feature ablation: baselines first, then CSD indicators
    feature_sets = [
        # Baselines
        ...
        (["toxicity_trend", "toxicity_mean", "toxicity_max",
          ...
          "lexical_divergence"],
         "+ Lexical divergence"),
        # Now add CSD indicators one by one
        (["toxicity_trend", "toxicity_mean", "toxicity_max",
          "sentiment_trend", "sentiment_mean", "sentiment_var",
...
def main():
    """Run CGA analysis experiments and save results."""
    np.random.seed(RANDOM_SEED)

    utt_df, summary_df = load_cga_features()
    print(f"Loaded {len(utt_df)} utterances from {utt_df['convo_id'].nunique()} conversations\n")

    exp4_results = experiment_4_csd_indicators(utt_df)
    exp5_results = experiment_5_incremental_regression(utt_df)

    all_results = {}
    all_results.update(exp4_results)
    all_results.update(exp5_results)

    output_path = RESULTS_DIR / "cga_analysis.json"
    with open(output_path, "w") as f:
        json.dump(all_results, f, indent=2, default=str)

    print(f"\nAll CGA results saved to {output_path}")
